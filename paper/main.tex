...
Second, binning the mixed-length run (\texttt{oeis\_results.csv}; a different
population) by available terms gives true-holonomic revision rates
...
\url{https://github.com/sabilashang/where-induction-runs-out}.
% Fill in the archived DOI after the Zenodo deposit, and mirror it into
% CITATION.cff (which carries the same placeholder).
An archived release is deposited at Zenodo (DOI to follow).
...
vertical: the discovery point $\nd$. Both axes are logarithmic in the vertical
direction. Non-holonomic sequences (rightmost column) never separate from the
